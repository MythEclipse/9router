...
export async function cancelProviderResponseBody(providerResponse) {
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, cavemanEnabled, cavemanLevel, sourceFormatOverride, providerThinking }) {
  ...
  // Handle auth failures - try token refresh unless provider marks response as non-refreshable
  if (!executor.noAuth && executor.shouldRefreshCredentials(providerResponse)) {
    try {
      ...
      if (newCredentials?.accessToken || newCredentials?.copilotToken) {
        ...
        try {
          ...
          if (retryResult.response.ok) {
            await cancelProviderResponseBody(providerResponse);
